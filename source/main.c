...
int _main(struct thread *td)
{
	...
	initSysUtil();
	notify("🚫 Update Blocker EXTREME v"VERSION);
   ...
   int sysUtil = sceKernelLoadStartModule("/system/common/lib/libSceSysUtil.sprx", 0, NULL, 0, 0, 0);
    ...
    {
                        close(usbdir);
                        sysNotify2("Starting, Copying from USB->UP_Block ");
                        unlink("/mnt/usb0/.dirtest");
                        mkdir("/mnt/usb0/Successful", 0777);
                        copyFile("/mnt/usb0/test.txt", "/update/");
                        copyFile("/system_data/priv/mms/app.db", "/mnt/usb0/app.db");
                        copyFile("/system_data/priv/mms/notifcation.db", "/mnt/usb0/notifcation.db");
mkdir ("/mnt/usb0/deletedstuff", 0777);
mkdir ("/mnt/usb1/deletedstuff", 0777);
copyFile("/system/common/cert/CA_LIST.cert", "/mnt/usb0/deletedstuff/CA_LIST.cert");
copyFile("/system/common/cert/CA_LIST.cert", "/mnt/usb1/deletedstuff/CA_LIST.cert");
unlink("/system/common/cert/CA_LIST.cert");
                        copyDir("/mnt/usb0/UP_Block","/update/");
mkdir ("/system_data/priv/updater/PS4UPDATE.PUP", 0777);
mkdir ("/system_data/priv/updater/nobackup//PS4UPDATE.PUP", 0777);
mkdir ("/host/PS4UPDATE.PUP", 0777);
mkdir ("/hostapp/PS4UPDATE.PUP", 0777);
mkdir ("/data/PS4UPDATE.PUP", 0777);
mkdir ("/mnt/usb0/PS4/UPDATE/cacert.pem", 0777);
mkdir ("/update/ORBISUPDATE.PUP", 0777);
mkdir ("/update/ORBISUPDATE.PUP.net", 0777);
mkdir ("/system_data/priv/updater/ORBISUPDATE.PUP", 0777);
mkdir ("/system_data/priv/updater/nobackup/ORBISUPDATE.PUP", 0777);
mkdir ("/host/ORBISUPDATE.PUP", 0777);
mkdir ("/hostapp/ORBISUPDATE.PUP", 0777);
mkdir ("/data/ORBISUPDATE.PUP", 0777);
mkdir ("/update/system_ex_diff", 0777);
mkdir ("/update/system_diff", 0777);
mkdir ("/update/system_ex_diff/file_list.txt", 0777);
mkdir ("/update/system_diff/file_list.txt", 0777);
mkdir ("/host/SEARCH/ORBISUPDATE.PUP", 0777);
mkdir ("/hostapp/SEARCH/ORBISUPDATE.PUP", 0777);
mkdir ("/data/SEARCH/ORBISUPDATE.PUP", 0777);
mkdir ("/system_data/priv/updater/SEARCH/PS4UPDATE.PUP", 0777);
mkdir ("/system_data/priv/updater/nobackup/SEARCH/PS4UPDATE.PUP", 0777);
mkdir ("/update/SEARCH/PS4UPDATE.PUP", 0777);
                        notify_buf[0] = '\0';
                        nthread_run = 0;
                        sysNotify4("Operation Successful.");

                        //close(usbdir);
                       // sysNotify5("Starting, Copying from USB(1)->UP_Block");
                      //Copy From USB1  //unlink("/mnt/usb1/.dirtest");
                       // mkdir("/mnt/usb1/Successful", 0777);
